...
int GetNeededLogsFor(TileType tileType);
int GetNeededRocksFor(TileType tileType);
...
void OnTickUnitSawMill(Unit& unit)
{
	...
	if (unit.CurrentBehavior == UnitBehavior::Idle)
	{
		if (unit.Logs >= GetMaxLogsFor(unit) / 2 && jobTile.Logs < Grid::GetMaxLogsStored(jobTile))
		{
			...
			unit.TargetTile = grid.GetTilePosition(unit.JobTileIndex);
			unit.SetBehavior(UnitBehavior::Moving);
		}

		// Check if there is a full tree
		auto treePositions = grid.GetTiles(TileType::Tree, grid.GetTilePosition(unit.JobTileIndex), 3);
	...
	{
		Tile& tile = grid.GetTile(unit.TargetTile);
...
void OnTickUnitBuilderHut(Unit& unit)
{
	if (unit.CurrentBehavior == UnitBehavior::Idle)
	{
		...
		if (unit.Logs > 0 || unit.Rocks > 0)
		{
			...
			auto storagePositions = grid.GetTiles(TileType::Storage, grid.GetTilePosition(unit.JobTileIndex), 5);
			Tile& mayorHouseTile = grid.GetTile(grid.GetTiles(TileType::MayorHouse)[0]);

			if (unit.Logs > 0)
			{
				if (mayorHouseTile.Logs < Grid::GetMaxLogsStored(mayorHouseTile))
				{
					unit.TargetTile = grid.GetTiles(TileType::MayorHouse)[0];
					unit.SetBehavior(UnitBehavior::Moving);
				}
				else
				{
					...
					{
						auto& storageTile = grid.GetTile(storagePosition);
...
void onTickUnitLogistician(Unit& unit)
{
	if (unit.CurrentBehavior == UnitBehavior::Idle)
	{
		...
		Tile& construction = grid.GetTile(constructionPosition);
		...
		if (unit.Logs > 0 || unit.Rocks > 0)
		{
			grid.ForEachTile([&](Tile& tile, TilePosition position)
			{
				if ((tile.Type == TileType::Storage || tile.Type == TileType::LogisticsCenter || tile.Type == TileType::MayorHouse) && unit.Logs > 0 ? tile.Logs < Grid::GetMaxLogsStored(tile) : tile.Rocks <
					Grid::GetMaxRocksStored(tile))
				{
					unit.TargetTile = position;
					unit.SetBehavior(UnitBehavior::Moving);
		...
		auto sawmillPositions = grid.GetTiles(TileType::Sawmill);

		for (auto& sawmillPosition : sawmillPositions)
		{
			auto& sawmillTile = grid.GetTile(sawmillPosition);
		...
		auto quarryPositions = grid.GetTiles(TileType::Quarry);

		for (auto& quarryPosition : quarryPositions)
		{
			auto& quarryTile = grid.GetTile(quarryPosition);
...
void GenerateMap()
{
	...
	for (int i = 0; i < 3; i++)
	{
		units.push_back(Unit{.Position = grid.ToWorldPosition(grid.GetTiles(TileType::MayorHouse)[0]) + Vector2F{ Random::Range(0, 25), Random::Range(0, 25) }});
	}
}
